module Highrise
  class CurlHelper
    ...
    def get_document_from_id(collection_name_and_id)
      return get_document("#{Highrise::Base.site.to_s}#{collection_name_and_id}") if Highrise::Base.user.nil?
      get_document("#{Highrise::Base.site.to_s.sub(/:\/\//, "://#{Highrise::Base.user.to_s}:X@")}#{collection_name_and_id}")
    end
